...
{
  const uint8_t line = (x - 1) * 8;
  const uint8_t column = (y - 1) * 16;
  OLED_PrintASCIIChar(line, column, ch, &afont16x8, OLED_COLOR_NORMAL); // 经测试，0.96寸oled分块区恰好为16*8，故只用font16x8,若使用其他尺寸oled，要更改afont和compenstate
  OLED_ShowFrame();
}
...
void OLED_ShowString(uint8_t y, uint8_t x, char *str)
{
  const uint8_t line = (x - 1) * 8;
  const uint8_t column = (y - 1) * 16;
  OLED_PrintString(line, column, str, &font16x16, OLED_COLOR_NORMAL);
  OLED_ShowFrame();
}
...
void OLED_Test()
{
  while(1)
  {
    ...
    for (i = 0; i < 15; i++)
    {
      OLED_ShowChar(1, 1 + i, 'A' + i);
    }

    for (i = 0; i <15; i++)
    {
      OLED_ShowChar(2, 15 - i, 'Z' - i);
    }

    for (i = 0; i < 15; i++)
    {
      OLED_ShowChar(3, 1 + i, 'A' + i);
    }

    for (i = 0; i < 15; i++)
    {
      OLED_ShowChar(4, 15 - i, 'Z' - i);
    }
    ...
    for (i = 0; i < 4; i++)
    {
      OLED_ShowChar(1 + i, 1, '1' + i);
      OLED_ShowChar(1 + i, 5, '1' + i);
      OLED_ShowChar(1 + i, 9, '1' + i);
      OLED_ShowChar(1 + i, 13, '1' + i);
    }
...
void OLED_Test()
{
  ...
  {
    OLED_DrawCircle(64, 32, 8 * i, OLED_COLOR_NORMAL);
    HAL_Delay(200);
  }
  OLED_NewFrame();
  HAL_Delay(200);
  OLED_DrawImage(30, 1, &snakeImg, OLED_COLOR_NORMAL);
